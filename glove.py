import json
selected_data = ['1', '3', '5', '8']
#train_data_subset = {n: data for n, data in train_data.items() if n in selected_data}
...
for word in vocab:
    ...
    if word in glove_wv:
        danci=glove_wv[word]
        embedding_matrix[word_idx, :] = torch.from_numpy(danci)
...
class FFTextClassifier(nn.Module):
    ...
    def __init__(self, vocab_size, sequence_length, embedding_size, hidden_size, num_classes):
        ...
        self.hidden_layer = nn.Linear(self.embedding_size*sequence_length, hidden_size)  # Full connection layer
    ...
    def forward(self, input_words):
        ...
        embedded_words = embedded_words.reshape(embedded_words.shape[0], self.sequence_length*self.embedding_size)
