...
def runlstmmodel(train_data,dev_data,texts,dev_texts,cluster_num,hidden_size,num_epochs):
...
#    print(emb_model.wv.key_to_index['Brexit'])
    ...
    class LSTMTextClassifier(nn.Module):
        ...
        def __init__(self, vocab_size, sequence_length, embedding_size, hidden_size):
            ...
            # self.embedding.weight = nn.Parameter(weights)
            self.lstm_layer = nn.LSTM(input_size=embedding_size, hidden_size=hidden_size, batch_first=True,bidirectional=True )  # Hidden layer
    ...
    all_dev_output = train_nn(num_epochs, ff_classifier_model, train_loader, dev_loader)
    return all_dev_output

    plt.title("softlabel")
    plt.plot(dev_losses)
    plt.xlabel('epochs')
    plt.ylabel('loss')
    plt.title(f'loss for cluster{cluster_num} using bidirectional lstm')
    plt.show()
