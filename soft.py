...
def soft_solution(data):
  ...
  for n in data:
    ...
    label_one = data[n]['soft_label']['0']
    label_zero = data[n]['soft_label']['1']
    new_label = [label_one, label_zero]
    soft_list.append(new_label)
  return soft_list
devlabel_list=[]
for n in dev_data:
    label_one=dev_data[n]['soft_label']['1']
    devlabel_list.append(label_one)
...
def f1_metric(targets, prediction):
    f1_wa = metrics.f1_score(targets, prediction, average = 'micro')
    return f1_wa
...
def train_nn(num_epochs, model, train_dataloader, dev_dataloader):
 ...
 for e in range(num_epochs):
  ...
  for dev_input_ids, dev_labels in dev_dataloader:
            ...
            predicted_labels = torch.argmax(dev_output, dim=1)
            #dev_predictions.append(predicted_labels.numpy)
            predicted=predicted_labels.numpy()
            labels = dev_labels[:, 1]
            #labels_list.append(labels.numpy())
            dev_f1 = f1_score(labels.numpy(), predicted, average='macro')
            all_dev_outputs.append(dev_output)
            #dev_precision = precision_score(labels.numpy(), predicted, average='macro')
  #dev_recall = recall_score(dev_labels, dev_predictions, average='macro')
  #这一段计算precision的
  all_dev_outputs = torch.cat(all_dev_outputs, dim=0)
  #predicted_labels = torch.argmax(all_dev_outputs, dim=1).numpy()
  #用argmax 全部预测为class 0了
  #dev_label_list = np.array(devlabel_list)

  # = precision_score(dev_label_list, predicted_labels, average='macro')

  dev_predictions.append(dev_f1)
  dev_losses.append(np.mean(dev_losses_epoch))
  print("Epoch: {}/{}".format((e + 1), num_epochs),
                  "Validation Loss: {:.4f}".format(dev_losses[-1]),
  "F1 Score: {:.4f}".format(dev_predictions[-1]))
        #"Validation Recall: {:.4f}".format(dev_recall))

 return model, dev_losses
# Calculate precision using soft labels directly
# Compute precision
#precision = precision_score(dev_label_list, predicted_labels, average='macro')
  #
...
plt.xlabel('epochs')
plt.ylabel('loss')
plt.title('loss using full connection layer')
plt.show()
